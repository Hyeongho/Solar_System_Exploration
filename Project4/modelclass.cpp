...
bool ModelClass::Initialize(ID3D11Device* device, char* modelFilename, WCHAR* textureFilename, int modelNumber)
{
	bool result;
	...
	result = LoadDataStructures(modelFilename, modelNumber);
	if (!result)
	{
		return false;
	}
...
bool ModelClass::LoadDataStructures(char* filename, int filenumber)
{
