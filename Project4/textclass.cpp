	...
	screenWidth, int screenHeight, D3DXMATRIX baseViewMatrix)
{
	...
	result = m_Font->Initialize(device, (char*)"../Project4/data/fontdata.txt", (WCHAR*)L"../Project4/data/font.dds");
	if (!result)
	{
	...
	result = UpdateSentence(m_sentence1, (char*)"FPS", 100, 100, 1.0f, 1.0f, 1.0f, deviceContext);
	if (!result)
	{
		return false;
	}
	...
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence2, (char*)"CPU", 100, 200, 1.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
	...
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence3, (char*)"polygons used", 100, 300, 1.0f, 0.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
	...
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence4, (char*)"distance from the earth", 100, 400, 1.0f, 0.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
	// Initialize the first sentence.
	result = InitializeSentence(&m_sentence4_1, 32, device);
	if (!result)
	{
		return false;
	}
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence4_1, (char*)"erath distance", 100, 450, 1.0f, 0.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
	...
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence5, (char*)"Ship Speed", 100, 500, 1.0f, 0.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
	// Initialize the first sentence.
	result = InitializeSentence(&m_sentence5_1, 32, device);
	if (!result)
	{
		return false;
	}
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence5_1, (char*)"Ship Speed", 100, 550, 1.0f, 0.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
	...
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence6, (char*)"Life", 100, 600, 1.0f, 0.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
	...
	orthoMatrix)
{
	...
	result = RenderSentence(deviceContext, m_sentence4_1, worldMatrix, orthoMatrix);
	if (!result)
	{
		return false;
	}
	...
	result = RenderSentence(deviceContext, m_sentence5_1, worldMatrix, orthoMatrix);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetPolygon(int polygonCount, ID3D11DeviceContext* deviceContext)
{
	char tempString[16];
	char ploygonString[16];
	bool result;
	// Convert the cpu integer to string format.
	_itoa_s(polygonCount, tempString, 10);
	// Setup the cpu string.
	strcpy_s(ploygonString, "Polygons: ");
	strcat_s(ploygonString, tempString);

	result = UpdateSentence(m_sentence3, ploygonString, 20, 60, 1.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetEarthDistance(D3DXVECTOR3 earthDistance, ID3D11DeviceContext* deviceContext)
{
	char tempString[32];
	char distanceString1[32];	// 지구와의 거리 설명글
	char distanceString2[32];	// 실제 거리
	bool result;

	// Setup the cpu string.
	strcpy_s(distanceString1, "Distance from the Earth: ");

	result = UpdateSentence(m_sentence4, distanceString1, m_screenWidth - 150, m_screenHeight - 120, 1.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
	...
	// Convert the cpu integer to string format.
	_itoa_s(D3DXVec3Length(&earthDistance), tempString, 10);
	// Setup the cpu string.
	strcpy_s(distanceString2, tempString);
	strcat_s(distanceString2, "  (10000 km)");


	result = UpdateSentence(m_sentence4_1, distanceString2, m_screenWidth - 140, m_screenHeight - 100, 1.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
